/**
 * Azure Storage Queue input - Input Form Hook
 *
 * Handles the encrypted-field UX problem for the `sas_token` field (same
 * pattern as the Demo Gen TA's hec_token hook):
 *
 *   - `sas_token` is `required: true` + `encrypted: true`. UCC renders
 ...
 *     the user retypes the entire SAS -- so editing any OTHER field (e.g.
 *     bumping the interval) would force the user to paste the SAS again.
 *
 * Fix:
 *   1. onEditLoad() pre-fills `sas_token` with "******" so the required-field
 *      validator passes when the user clicks Update without touching it.
 *   2. onSave(dataDict) strips `sas_token` from the outbound PATCH IF it still
 ...
 * sas_token field must NOT carry a minLength >= 7 validator -- it doesn't.)
 *

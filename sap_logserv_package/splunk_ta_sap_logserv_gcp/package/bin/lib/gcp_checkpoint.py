"""
gcp_checkpoint.py - Object dedup checkpoint sidecar.

Pub/Sub delivery is at-least-once: a message can be redelivered if the ack
deadline lapses before we acknowledge it, or if the input process dies
after emitting an object's events but before acking the message. Without dedup,
that redelivery re-ingests the object -> duplicate events.

This sidecar records the per-object-version key (bucket/name#generation) of
every object successfully ingested. Before fetching a blob we check `seen()`; after
emitting we `record()`. Redelivery of an already-ingested object is then a no-op
(we still ack the message).
...
class DedupCheckpoint:
    """Bounded, TTL-pruned set of ingested object keys, backed by a JSON file."""

"""
additional_packaging.py - UCC post-build hook for splunk_ta_sap_logserv_gcp.

This TA carries ONLY the sap_logserv_gcp_pubsub modular input (the Azure twin of the
...
1. Patch EVERY [admin_external:*] stanza in restmap.conf to carry handlertype /
   ...
   stanza here is UCC's auto-generated sap_logserv_gcp_pubsub input-CRUD REST handler.
...
2. Patch the generated inputs.conf [sap_logserv_gcp_pubsub] kind stanza to carry
   python.version = python3, python.required = 3.13, and
   _meta = cloud_provider::gcp. UCC 6.1.0 emits this stanza with an EMPTY body,
   so all three are added. python.required clears
   check_modular_inputs_python_required; _meta tags every event from any
   instance of this input with the indexed cloud_provider=gcp field (per-input
...
3. Overwrite UCC's auto-generated bin/sap_logserv_gcp_pubsub.py stub with our real
...
# The sap_logserv_gcp_pubsub is, by definition, an GCP ingest path, so stamp every
# event from it with the indexed cloud_provider=gcp field via the kind-stanza
# _meta. (Proven session-045 pattern; survives all downstream transforms.)
AZURE_INPUT_KIND = "[sap_logserv_gcp_pubsub]"
GCP_META_LINE = "_meta = cloud_provider::gcp\n"
...
def cleanup_output_files(output_path, ta_name):
    ...
    # --- 2. Patch inputs.conf: [sap_logserv_gcp_pubsub] python.required + _meta ---
    patch_inputs_conf(inputs_path)

    # --- 3. Overwrite UCC's auto-generated bin/sap_logserv_gcp_pubsub.py stub ---
    ta_source_root = os.path.dirname(os.path.abspath(__file__))
    src_input_py = os.path.join(
        ta_source_root, "package", "bin", "sap_logserv_gcp_pubsub.py")
    dst_input_py = os.path.join(app_dir, "bin", "sap_logserv_gcp_pubsub.py")
...
def patch_inputs_conf(inputs_path):
    ...
    - The [sap_logserv_gcp_pubsub] modular-input kind stanza: ensure it carries
      python.version = python3, python.required = 3.13, and
      _meta = cloud_provider::gcp. UCC 6.1.0 emits this stanza with an EMPTY
    ...
    if not saw_azure_kind:
        ...
        out.append(
            "\n{}\npython.version = python3\npython.required = 3.13\n{}".format(
                AZURE_INPUT_KIND, GCP_META_LINE))

    with open(inputs_path, "w") as f:
        f.writelines(out)
    print("[additional_packaging] Patched inputs.conf: [sap_logserv_gcp_pubsub] "
          "python.required + _meta = cloud_provider::gcp.")

...
- **Multi-cloud ingest** — supports both **AWS S3** (via the Splunk Add-on for AWS) and **Microsoft Azure Blob Storage** (via the Splunk Add-on for Microsoft Cloud Services). Same downstream sourcetypes, dashboards, ES integration, and AI Assistant regardless of ingest channel. Every event carries an indexed `cloud_provider` field (`aws` or `azure`) for cross-cloud reporting; the **Multi-Cloud Overview** dashboard surfaces the per-provider split. See [Azure Setup Guide](content/install-setup/azure-setup.md).

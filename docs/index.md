...
- **Multi-cloud ingest** — supports **AWS S3** (via the Splunk Add-on for AWS), **Microsoft Azure Blob Storage** (via the first-party **LogServ Azure add-on**, `splunk_ta_sap_logserv_azure`), and **Google Cloud Storage** (via the first-party **LogServ GCP add-on**, `splunk_ta_sap_logserv_gcp`) — the cloud add-ons install per Heavy Forwarder. Same downstream sourcetypes, dashboards, ES integration, and AI Assistant regardless of ingest channel. Every event carries an indexed `cloud_provider` field (`aws`, `azure`, or `gcp`) for cross-cloud reporting; the **Multi-Cloud Overview** dashboard surfaces the per-provider split. See the [Azure Setup Guide](content/install-setup/azure-setup.md) and the [GCP Setup Guide](content/install-setup/gcp-setup.md).

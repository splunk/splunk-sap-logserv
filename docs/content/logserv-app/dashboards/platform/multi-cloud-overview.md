...
- **Azure Events** -- Event count where `cloud_provider = "azure"`. Sparkline shows daily Azure-only volume. On an install without Azure ingest, this KPI shows `0` and its sparkline is empty.
- **GCP Events** -- Event count where `cloud_provider = "gcp"`. Sparkline shows daily GCP-only volume. On an install without GCP ingest, this KPI shows `0` and its sparkline is empty.
...
Panels read the `logserv_mc_rollup` KV Store collection — metric `count` carries per-hour event
counts by (cloud provider, sourcetype) and metric `hosts` the per-provider host sets (kept separate
so distinct-host counts stay correct when an event carries a multivalue host). Populated at minute
:22 of every hour by `logserv_mc_aggregate` over the entire index, keyed on the **`cloud_provider`
indexed field** stamped at ingest (see Required Configuration below); events with no provider
attribution are counted as `aws` by convention.

Summary-backed panels switch automatically to their exact raw-equivalent search when the selected window is shorter than 90 minutes, so sub-hour investigation stays real-time. The collection keeps 365 days of hourly history (trimmed by a daily retention search); a fresh install seeds the last 30 days from **Settings → Dashboard Data → Run backfill**. Unlike the other dashboards, this one is exempt from the global **Cloud** dropdown — its panels pin their own provider values (AWS / Azure / GCP side by side), so a global filter would be circular. Full schedule and freshness reference: [Performance & Data Freshness](../performance.md).
...
- **Total Events ≠ sum of the per-provider KPIs** -- Total Events counts every event regardless of `cloud_provider` value, while the three per-provider KPIs match `aws` / `azure` / `gcp` exactly. If Total exceeds their sum, some events carry a different `cloud_provider` value — run ``| tstats count WHERE `sap_logserv_idx_macro` BY cloud_provider`` to see it. If instead the AWS KPI under-reports, check that the `sap_logserv_cloud_provider_default_macro` macro (which defaults legacy un-attributed events to `aws`) is loaded in your search head's `macros.conf` — the LogServ App ships it in `default/macros.conf`; confirm with `| rest /services/configs/conf-macros | search title=sap_logserv_cloud_provider_default_macro`.

## :material-circle-box:{ .taiconcolor } Drill-Down Behavior

Every KPI sparkline and the Daily Event Volume chart respect the dashboard's global time-range picker + the per-dashboard Refresh picker like any other LogServ App dashboard. Unlike other dashboards, this one has no Cloud Provider picker in its title row — the provider split *is* the dashboard, so every panel reports all providers side by side.

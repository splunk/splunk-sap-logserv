...
1. **For Azure ingest:** the `_meta = cloud_provider::azure` line is present on the `[sap_logserv_azure_queue://...]` stanza automatically (the LogServ Azure add-on injects it). See [Azure Setup Guide](../../../install-setup/azure-queue-setup.md).
...
See [Azure Setup Guide](../../../install-setup/azure-queue-setup.md) for the canonical setup procedure (including the `cloud_provider` attribution pattern that applies symmetrically to both clouds).

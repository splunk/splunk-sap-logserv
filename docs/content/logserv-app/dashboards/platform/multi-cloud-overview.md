...
- **Stalled ingest channel** -- If the Azure KPI sparkline goes flat-zero for the most recent 1–3 days while the AWS sparkline continues to climb (or vice versa), the affected channel's add-on input has likely stalled. Check the **Splunk Add-on for Microsoft Cloud Services** input on the Heavy Forwarder (or the `Splunk_TA_aws` S3 input for AWS) — the internal log line for stuck inputs is in `index=_internal sourcetype=splunkd component=...` (e.g., `ModularInputs` for the Azure add-on, `S3Input` for AWS).
...
1. **For Azure ingest:** the `_meta = cloud_provider::azure` line must be present on the `[mscs_storage_blob://...]` input stanza in the Splunk Add-on for Microsoft Cloud Services. See [Azure Setup Guide](../../../install-setup/azure-setup.md).

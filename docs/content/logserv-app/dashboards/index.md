...
| [Multi-Cloud Overview](platform/multi-cloud-overview.md) | Per-cloud-provider ingest split: AWS vs Azure event counts, sourcetype distribution per provider, recent activity. Built on the indexed `cloud_provider` field with the `sap_logserv_cloud_provider_default_macro` for legacy AWS defaulting. See [Azure Setup Guide](../../install-setup/azure-queue-setup.md). | All sourcetypes (filtered by `cloud_provider`) |
